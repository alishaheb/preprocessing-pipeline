...
def clean_columns(df: pd.DataFrame) -> pd.DataFrame:
    # strip whitespace, tabs, and unify spaces
    df = df.copy()
...
class DataConfig:
    ...
    sep: str = ";"  # the dataset is semicolon-separated
...
class DatasetLoader:
    ...
    def load(self) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        # Defensive: ensure numeric dtypes (dataset looks numeric already)
        # If any non-numeric sneaks in, let SimpleImputer handle and models complain clearly.
        return X, y


# ---------- Preprocessing ----------
class PreprocessorFactory:
    """Creates preprocessing transformers with/without scaling."""
...
# ---------- Models ----------
class ModelFactory:
    """Creates 5 model pipelines with appropriate preprocessing."""
    ...
    def build_all(self) -> Dict[str, Pipeline]:
        ...
        # 3) Gaussian Naive Bayes (no scaling required, continuous features)
        models["GaussianNB"] = Pipeline([
            ("prep", self.pre_no_scale),
            ("clf", GaussianNB())
        ])

        # 4) MLP (benefits from scaling)
        models["MLP"] = Pipeline([
            ("prep", self.pre_scale),
            ("clf", MLPClassifier(
